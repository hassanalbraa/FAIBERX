...
function UserOrdersContent() {
  ...
  const { data: rawOrders, isLoading: isOrdersLoading } = useCollection<Order>(userOrdersQuery);
                    ...
                    <TableCell className="text-right">{order.total.toFixed(2)} SDG</TableCell>
                     <TableCell className="text-center">
                        <Button asChild variant="outline" size="sm">
                            <Link href={`/account/orders/${order.id}`}>عرض</Link>
                        </Button>
                    </TableCell>
                  </TableRow>
                ))}
  ...
  return (
    <div className="container mx-auto px-4 py-8 md:py-12">
      <div className="mb-12">
        ...
        <p className="text-muted-foreground mt-2">عرض حالة وتفاصيل جميع طلباتك السابقة.</p>
      </div>
      <UserOrdersContent />
    </div>
  );
