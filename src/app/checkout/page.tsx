...
export default function CheckoutPage() {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    const orderData = {
      ...
      items: cartItems.map(item => ({ productId: item.product.id, name: item.product.name, image: item.product.image, quantity: item.quantity, price: item.product.price, size: item.size || "افتراضي" })),
      shippingAddress: { name: `${values.firstName} ${values.lastName}`, email: values.email, address: values.address, city: values.city, country: values.country, whatsappNumber: values.whatsappNumber },
        ...
        <div>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
              <Card>
                <CardHeader><CardTitle>معلومات الشحن</CardTitle><CardDescription>اختر عنوان محفوظ أو أدخل عنوان جديد.</CardDescription></CardHeader>
              ...
              <Card>
                <CardHeader><CardTitle>تفاصيل الدفع</CardTitle><CardDescription>أكمل التحويل البنكي ثم أدخل رقم العملية.</CardDescription></CardHeader>
                <CardContent className="space-y-4">
